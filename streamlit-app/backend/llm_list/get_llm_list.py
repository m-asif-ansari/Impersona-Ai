...
def get_llm_from_groq() -> list:
    ...
    try:
        ...
        logger.info("List of available LLMs from Groq API:")
        for i in model_list:
            logger.info(i)  # Log each model name

        # returning the model list variable
        return model_list
    except Exception as e:
        logger.error(f"An error occurred while fetching LLMs from Groq API: {e}")
        # If an error occurs, return pre-defined LLMs instead
        logger.info("Returning pre-defined LLMs instead.")
...
def get_llm_list() -> list:
    ...
    logger.info("List of pre-configured LLMs:")
...
if __name__ == "__main__":
    print("List of available models - ", get_llm_from_groq)
